...
| OG-03 | Zero invented panel prices | Tools + loopback `/calc` + prompt hard rule + goldens | DONE | SuperAgent engine parity tested (IMP-07 #745) |
...
| OG-05 | Full tool catalog documented for MCP/GPT | Local 55 + OpenAPI + manifest; prod **55**; contract pin test | DONE | IMP-01 residual #748 pin 55 |
...
| OG-09 | Measurable cost $/day | `costTelemetry` + SuperAgent `logAgentCost`; SSE `done` latency; Omni cap 50 | PARTIAL | Query path DONE; SSE latency DONE (#748); hub UI still open |
...
| OG-11 | Goldens + eval gate releases | **22** goldens + `GOLDEN_REQUIRED=1` in pre-release | DONE | IMP-11 #746 closed; more packs optional |
...
| OG-15 | p95 first SSE token &lt; 2.5s | SSE `done` emits latency_ms/ttft_ms (IMP-12 #748); p95 baseline not yet ops-collected | PARTIAL | Collect 1-week baseline — IMP-12 residual |
...
| G-06 cost $/day | **PARTIAL** → query path + SSE latency closed; hub UI optional |
