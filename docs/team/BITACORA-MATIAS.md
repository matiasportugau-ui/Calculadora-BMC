...
- **2026-08-20** — Ident glory sting + landing `/`: BC gold-foil, LAM three-planks, SMART IMAX-air on Continuar. Sin login el ident es siempre el index. Deploy tenant Vercel (no BMC). No merge #1051.
- **2026-08-20** — Cloud Run `panelin-calc` rev `01035-n5w` con prompt tenant. Health 200; `/api/admin/tenants` 401 (ruta viva). WHITELABEL no seteado. **Next:** Cmd+Shift+R en las 3 calculadoras y preguntar “quién sos?”. No merge #1051.
- **2026-08-19** — Prompt por tenant: JenIA/MonkIA/Basuuuu IA no hablan BMC (compiler `tenantPrompt.js`, tools solo calc). Tests leak 0. **Pendiente:** deploy Cloud Run `panelin-calc` para que las tres URLs dejen de responder como Panelin. No merge #1051.
- **2026-08-19** — Tres calculadoras en prod con agente propio: BC JenIA (`calculadora-bc.vercel.app`), LAM MonkIA (`calculadora-paneleslam.vercel.app`), SmartBuilding Basuuuu IA (`calculadora-smartbuilding.vercel.app`). Migración eval aplicada. **Pendiente:** deploy Cloud Run `panelin-calc` para persistir transcripciones (el SPA tenant sigue pegándole al API BMC). No merge #1051. No rebuild BMC.
- **2026-08-19** — Tenant IA eval: JenIA (BC), MonkIA (LAM), Basuuuu IA (SmartBuilding). Transcripciones + tokens + USD estimado en tab IA (eval). Postgres identity.tenant_agent_*. **Next:** `npm run jenerik:migrate` y un chat de prueba. No merge #1051.
- **2026-08-19** — Control Tower v1 (Phase 0): luces en línea + tabs Live/Analytics (funnel wizard→PDF, tiempo en app estimado). APIs scoped por tenant. Tests tenant-analytics 5/5. **Next:** heartbeat Phase 1. No merge #1051.
- **2026-08-19** — Tenant control plane: `/hub/admin/tenants` + `/hub/admin/tenant/:slug` (Control / Cuentas / Monitor). Pause, revoke, export JSON/CSV. Silos por slug. Tests tenant-control 11/11. **Next:** `doppler run -- npm run dev:full` en `~/calculadora-bmc-jenerik` para ver datos vivos. No merge #1051.
- **2026-08-13** — Agregar producto visible de nuevo: tarjeta **NUEVO** en columna izquierda + catálogo por categoría restaurado + drawer controlado. Ship PR #1038 (extras HITL + search + PA5852 + workspace). **Next:** verificar en prod tras merge.
- **2026-08-09** — PEA M0–M4 **SHIPPED** main `#967` (`a723f666`): offline stack + gate/CI + SDD kit; flags OFF. Session closeout handoff: docs/team/HANDOFF-2026-08-09-0731-pea-pr967-closeout.md. **Next:** `/goal` orchestrator → live-probe phase 1.
- **2026-08-09** — 3D Logística glory campaign **LIVE** on main tip `3b858b6e`: #953 CSP Drive/Dropbox, #955 adjunto proxy-first, #960 multi-format autocarga parser, #962 cab nose −X + brighter lights. Sandbox SDD+source private repo `matiasportugau-ui/3d-logistic-viewer` (`5dbaeb0`). Handoff: docs/team/HANDOFF-2026-08-09-3d-logistica-glory.md.
