- **2026-08-31** — Panelin install KB increment (append-only). `proceso-constructivo.md` + encuentros/fichas/FAQ. Lessons already on GCS. Original fases/tablas intact. Cloud Run needs `data/knowledge` in the image (`workflow_dispatch` deploy-calc-api — path filter skips that folder).
- **2026-08-26** — Productos sueltos = catálogo Agregar producto (búsqueda, chips, fotos, qty, Agregar) inline en presupuesto libre. Paneles por medidas + Fuera de lista se mantienen. Branch `feat/prods-sueltos-catalog-ui`.
