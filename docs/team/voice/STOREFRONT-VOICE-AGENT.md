...
Architecture SoT: `docs/sdd/storefront-voice-agent/SDD.md`. Policy: sell loop (ficha → cart or aproximación); **offer** quote when the project is buyable (do not wait for insist); lista web + PDF; never flete; Admin 2.0 `origen=VW`.
...
| Identify | `POST /api/public/voice/identify` (name + phone → Admin 2.0 row, origen `VW`) |
| Chat log | `POST /api/public/voice/log` + each `/chat` (transcript → col J of the same row) |
| Text chat | `POST /api/public/voice/chat` (also persists Admin col J when `adminRow` is sent) |
...
| Shared brain | Public-safe slice of IAlfred↔Panelin `lessons.json` via `storefrontBrain.js`. Operator Admin/Drive lessons never enter the pack. |
