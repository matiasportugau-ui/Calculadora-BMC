...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Date**                           | 2026-04-19                                                                                                                                                                                                                                                                       |
| **Focus (one line)**               | **cm-2 ingest real** — intento `email:ingest-snapshot --limit 1` → prod devolvió **504** (timeout Cloud Run / ruta pesada). Reintentar con API local o subir timeout en despliegue.                                                                                        |
| **Energy / time box**              | —                                                                                                                                                                                                                                                                                |
...
- 2026-04-19: **Ingest correo (prod)** — `BMC_API_BASE=https://panelin-calc-642127786762.us-central1.run.app npm run email:ingest-snapshot -- --limit 1` → **HTTP 504** en `POST /api/crm/ingest-email` (sin fila CRM). **cm-2** sigue `doing`. Próximo: reintentar con `npm run start:api` + mismo comando sin `BMC_API_BASE`, o revisar timeout Cloud Run / logs.
