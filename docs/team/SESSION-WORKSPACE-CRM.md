...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Date**                           | 2026-04-19                                                                                                                                                                                                                                                                       |
| **Focus (one line)**               | **cm-2** — ingest **1** mensaje OK vía **API local**; prod **504** en mismo POST (timeout). Validar fila CRM en Sheets; prod requiere timeout/despliegue.                                                                                        |
| **Energy / time box**              | —                                                                                                                                                                                                                                                                                |
...
- 2026-04-19: **Ingest correo** — **Prod:** `BMC_API_BASE=…run.app npm run email:ingest-snapshot -- --limit 1` → **HTTP 504** (`POST /api/crm/ingest-email`, timeout típico Cloud Run). **Local** (`BMC_API_BASE=http://localhost:3001`, API ya en `:3001`): **1 ok** (~32s), dedupe en `.email-ingest/processed-ids.json`. Revisar fila en **CRM_Operativo** en planilla; para prod: subir timeout del servicio o optimizar ruta.
