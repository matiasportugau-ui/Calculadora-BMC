...
import { getFallbackAssistantReply } from "../services/fallback-llm.server";
  ...
  try {
    ...
    while (finalMessage.stop_reason !== "end_turn") {
      try {
        ...
        console.error("Primary model stream failed, using fallback:", primaryModelError);
        const fallbackText = await getFallbackAssistantReply(userMessage);
        stream.sendMessage({ type: "fallback_notice", notice: AppConfig.errorMessages.fallbackUsed });
        stream.sendMessage({ type: "chunk", chunk: fallbackText });
        stream.sendMessage({ type: "message_complete" });
        const assistantContent = [{ type: "text", text: fallbackText }];
        conversationHistory.push({ role: "assistant", content: assistantContent });
        await saveMessage(conversationId, "assistant", JSON.stringify(assistantContent)).catch((err) =>
