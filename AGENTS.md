...
| `npm run smoke:prod` | Smoke contra API pública (URL canónica en script): `GET /health`, `/capabilities`, chequeo `public_base_url`, `GET /auth/ml/status`, `POST /api/crm/suggest-response` (IA). `BMC_API_BASE` / `SMOKE_BASE_URL`; `-- --json` |
