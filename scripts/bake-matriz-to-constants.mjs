...
export const PRICE_ROOTS = new Set([
  "PANELS_TECHO", "PANELS_PARED", "FIJACIONES", "HERRAMIENTAS",
  "SELLADORES", "PERFIL_TECHO", "PERFIL_PARED", "SERVICIOS",
]);
export const PRICE_FIELDS = new Set(["venta", "web", "costo"]);

/** Parseo numérico del bake (coma decimal UY/US). Distinto de parseCsvNumber: admite negativos y no redondea. */
export function parseBakeNumber(v) {
...
/** CSV MATRIZ/export → Map path → { venta, web, costo }. */
export function wantedPricesFromCsvText(text) {
  const rows = parseCsvRows(text);
  if (!rows.length) return { wanted: new Map(), error: "csv_empty" };
  const headers = rows[0].map((h) => String(h || "").trim().toLowerCase().replace(/^\uFEFF/, ""));
  const iPath = colIdx(headers, "path");
  const iVenta = colIdx(headers, "venta_local", "venta_bmc_local", "venta_bmc", "venta");
  const iWeb = colIdx(headers, "venta_web");
  const iCosto = colIdx(headers, "costo");
  if (iPath < 0) return { wanted: new Map(), error: "csv_missing_path" };
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    if (!p) continue;
    wanted.set(p, {
      venta: iVenta >= 0 ? parseBakeNumber(rows[r][iVenta]) : null,
      web: iWeb >= 0 ? parseBakeNumber(rows[r][iWeb]) : null,
      costo: iCosto >= 0 ? parseBakeNumber(rows[r][iCosto]) : null,
    });
  }
  return { wanted, error: null };
}

/**
 * Scanner de constants.js: compara hojas {venta,web,costo} contra `wanted` (path → precios).
 * No escribe disco. `edits` son reemplazos numéricos listos para applyPriceEdits.
 */
export function collectConstantsPriceEdits(src, wanted) {
  const N = src.length;
  const stack = [];
  let lastToken = null;
  let currentKey = null;
  ...
  return { edits, touched, seenPaths, notFound };
}

export function applyPriceEdits(src, edits) {
  let out = src;
...
function main() {
  ...
  const { wanted, error } = wantedPricesFromCsvText(fs.readFileSync(path.resolve(csvArg), "utf8"));
  if (error === "csv_missing_path") {
    console.error("CSV sin columna 'path'.");
    process.exit(2);
  }

  const src = fs.readFileSync(CONSTANTS, "utf8");
  const { edits, touched, notFound } = collectConstantsPriceEdits(src, wanted);
  ...
  if (notFound.length) {
    console.log(`Paths del CSV sin hoja en constants.js: ${notFound.length}\n  ${notFound.slice(0, 20).join(", ")}${notFound.length > 20 ? " …" : ""}`);
  }
  ...
  if (dryRun) {
    ...
    fs.writeFileSync(CONSTANTS, applyPriceEdits(src, edits), "utf8");
