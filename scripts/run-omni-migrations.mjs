#!/usr/bin/env node
/**
 * Aplica migraciones SQL en orden desde server/migrations/omni/.
 * Uso: DATABASE_URL=postgres://... npm run omni:migrate
 *      OMNI_DATABASE_URL sobrescribe la URL si está definida.
 */
...
async function main() {
  ...
  if (!databaseUrl) {
    console.error("OMNI_DATABASE_URL or DATABASE_URL is required");
    process.exit(1);
  }
  ...
  try {
    ...
    for (const file of files) {
      const { rows } = await client.query("select 1 from omni_schema_migrations where name = $1", [file]);
      if (rows.length > 0) {
