...
| `scripts/mcp-panelin-http.mjs` | MCP server (stdio). Boots by fetching `/api/agent/tools-manifest`, registers all manifest-exposed tools dynamically, forwards calls to `/api/agent/exec-tool`. Forwards `BMC_API_TOKEN` as Bearer for the 4 write tools. Companion agent definition: `bmc-panelin-mcp.md`. |
