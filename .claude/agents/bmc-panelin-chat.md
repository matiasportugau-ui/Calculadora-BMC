...
| `server/lib/agentAuditLog.js` | Durable audit trail: every chat-loop tool call AND every `/api/agent/exec-tool` invocation emits one structured JSON line (`event="agent_tool_audit"`) to stdout. Cloud Logging captures it across cold-starts. Free-text inputs reduced to lengths; identifiers (telefono/rut/pdf_id) fingerprinted with SHA-256; bearer tokens fingerprinted as `mcp:<12-char>`; chat conversations as `chat:<conversationId>`. Filter: `jsonPayload.event="agent_tool_audit"`. |
