...
test("POST change-requests validates type/title; operators may propose knowledge", async () => {
  const inserts = [];
  const pool = {
    async query(sql, params = []) {
      if (sql.includes("SELECT count(*)::int AS n FROM panelin_workspace.workspaces")) {
        return { rows: [{ n: 1 }] };
      }
      if (sql.includes("INSERT INTO panelin_workspace.change_requests")) {
        inserts.push({ kind: "cr", params });
        return { rows: [], rowCount: 1 };
      }
      if (sql.includes("INSERT INTO panelin_workspace.knowledge_docs")) {
        inserts.push({ kind: "kb", params });
        return { rows: [], rowCount: 1 };
      }
      throw new Error(`Unexpected SQL: ${sql}`);
    },
  ...
  await withWorkspaceApp(
    ...
    async (port) => {
      const anon = await requestJson(port, "POST", "/api/workspace/change-requests", {
        body: { type: "knowledge", title: "Q?" },
      });
      assert.equal(anon.status, 401);

      const badType = await requestJson(port, "POST", "/api/workspace/change-requests", {
        token: accessToken(OPERATOR_ID),
        body: { type: "hack", title: "Q?" },
      });
      assert.equal(badType.status, 400);
      assert.equal(badType.body?.error, "invalid_type");

      const noTitle = await requestJson(port, "POST", "/api/workspace/change-requests", {
        token: accessToken(OPERATOR_ID),
        body: { type: "knowledge", title: "   " },
      });
      assert.equal(noTitle.status, 400);
      assert.equal(noTitle.body?.error, "title_required");

      const created = await requestJson(port, "POST", "/api/workspace/change-requests", {
        token: accessToken(OPERATOR_ID),
        body: {
          id: "CR-test-1",
          type: "knowledge",
          title: "Cuántos m2 por panel?",
          goodAnswer: "Usar calculadora /calc",
          workspaceId: "ws-1",
        },
      });
      assert.equal(created.status, 201, JSON.stringify(created.body));
      assert.equal(created.body?.ok, true);
      assert.equal(created.body?.changeRequest?.id, "CR-test-1");
      assert.equal(created.body?.changeRequest?.status, "proposed");
      assert.equal(created.body?.knowledgeDocId, "kb-cr-CR-test-1");
      assert.equal(inserts.some((i) => i.kind === "cr"), true);
      assert.equal(inserts.some((i) => i.kind === "kb"), true);
      assert.equal(inserts.find((i) => i.kind === "kb")?.params?.[0], "kb-cr-CR-test-1");
    },
  );
});
...
test("knowledge CR approve with PAOS_ENABLED=1 does not write active Training KB", async () => {
  const oldPaos = process.env.PAOS_ENABLED;
  process.env.PAOS_ENABLED = "1";

  const { __paosCandidatesTest } = await import("../server/lib/paosCandidates.js");
  const { __paosLedgerTest } = await import("../server/lib/paosEventLedger.js");
  ...
  const crs = {
    "cr-knowledge": {
      ...
      type: "knowledge",
      title: "Cuántos m2 por panel?",
      description: "Usar calculadora /calc",
      status: "pending",
      workspace_id: "ws-1",
      author_name: "ops",
      diff_text: "Usar calculadora /calc",
      diff_json: JSON.stringify({
        question: "Cuántos m2 por panel?",
        goodAnswer: "Usar calculadora /calc",
      }),
    },
  };
  ...
  const pool = {
    async query(sql, params = []) {
      ...
      if (sql.includes("UPDATE panelin_workspace.change_requests")) {
        const id = params[0];
        if (crs[id]) {
          crs[id] = { ...crs[id], status: "approved", reviewer_id: params[1] };
        }
        return { rows: [], rowCount: 1 };
      }
      if (sql.includes("INSERT INTO panelin_workspace.knowledge_docs")) {
        throw new Error("PAOS path must not insert knowledge_docs");
      }
      if (sql.includes("INSERT INTO panelin_workspace.telemetry_events")) {
        return { rows: [], rowCount: 1 };
      }
    ...
    await withWorkspaceApp(
      ...
      async (port) => {
        const res = await requestJson(
          port,
          "POST",
          "/api/workspace/change-requests/cr-knowledge/approve",
          { token: accessToken(SUPERADMIN_ID) },
        );
        assert.equal(res.status, 200);
        assert.equal(res.body?.ok, true);
        assert.equal(res.body?.status, "approved");
        assert.equal(res.body?.bmcKbId, null);
        assert.equal(res.body?.paos?.mode, "pending_approval_no_silent_kb");
        assert.ok(res.body?.paos?.candidateId);
        assert.equal(crs["cr-knowledge"].status, "approved");
      },
    );
  } finally {
    __paosCandidatesTest.reset();
    __paosLedgerTest.reset();
    if (oldPaos === undefined) delete process.env.PAOS_ENABLED;
    else process.env.PAOS_ENABLED = oldPaos;
  }
});
