...
async function run() {
  ...
  try {
    ...
    r = await fetch(`${base}/api/research/deep/resp_fake`);
    assert(
      "GET /api/research/deep/:id without auth → 401",
      r.status === 401,
      r.status,
      401
    );

    r = await fetch(`${base}/api/research/deep/resp_fake/cancel`, {
      method: "POST",
    });
    assert(
      "POST /api/research/deep/:id/cancel without auth → 401",
      r.status === 401,
      r.status,
      401
    );
