/**
 * Repartos API (#903/#921) — offline route regression.
 * Covers auth gates, DB-unavailable, revision conflict, immutable PUT,
 * confirm no_stops / re-confirm immutable, and create numbering without live Postgres.
 *
  ...
  return {
    async query(sql, params = []) {
      ...
      if (norm.startsWith("update repartos set") && norm.includes("payload = $2::jsonb")) {
        ...
        // Confirm path: atomic gate WHERE status = 'en_coordinacion' (#921)
        if (norm.includes("status = 'coordinado'")) {
          if (norm.includes("and status = 'en_coordinacion'") && row.status !== "en_coordinacion") {
...
test("POST confirm rejects empty stops and immutable closed/confirmed status", async () => {
  const pool = makeRepartosPool({
    repartos: [
      {
        id: "empty",
  ...
  await withServer({ pool }, async (port) => {
    ...
    // #921: canConfirmReparto blocks cerrado/coordinado with 409 immutable (not invalid_transition).
    const badClosed = await request(port, "POST", "/api/repartos/closed/confirm", {
      token: API_TOKEN,
      body: { actor: "tester" },
    });
    assert.equal(badClosed.status, 409);
    assert.equal(badClosed.body?.error, "immutable");

    const badConfirmed = await request(port, "POST", "/api/repartos/confirmed/confirm", {
      token: API_TOKEN,
      body: {
        actor: "attacker",
        payload: {
          stops: [{ id: "s1", cliente: "Hijacked", orderId: "999" }],
        },
      },
    });
    assert.equal(badConfirmed.status, 409);
    assert.equal(badConfirmed.body?.error, "immutable");
    const untouched = pool._rows.find((r) => r.id === "confirmed");
    assert.equal(untouched?.payload?.stops?.[0]?.cliente, "A");
    assert.equal(untouched?.revision, 4);
  });
});
...
test("POST confirm transitions en_coordinacion → coordinado and snapshots drivePlan", async () => {
  ...
  await withServer({ pool }, async (port) => {
    ...
    // Re-confirm must not rewrite the snapshot
    const again = await request(port, "POST", "/api/repartos/r-ok/confirm", {
      token: API_TOKEN,
      body: {
        actor: "ops2",
        payload: {
          stops: [{ id: "sX", cliente: "Overwrite", orderId: "0" }],
        },
      },
    });
    assert.equal(again.status, 409);
    assert.equal(again.body?.error, "immutable");
    assert.equal(pool._rows.find((r) => r.id === "r-ok")?.revision, 2);
    assert.equal(
      pool._rows.find((r) => r.id === "r-ok")?.payload?.stops?.length,
      2,
    );
  });
});
