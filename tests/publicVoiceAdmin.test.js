...
const log400 = storefrontActionLogPayload("shop_search", 400, { code: "shop_tool_client_only" });
assert.equal(log400["action.type"], "shop_search");
assert.equal(log400.actionType, "shop_search");
assert.equal(log400.status, 400);
assert.equal(log400.code, "shop_tool_client_only");
...
assert.match(actionSrc, /shop_tool_client_only/, "shop tools on /action are HTTP 400 with stable code");
