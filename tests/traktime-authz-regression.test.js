...
class PoolShim {
  ...
  async query(sql, params = []) {
    ...
    if (q.includes("from tk_tasks t") && q.includes("join tk_project_members")) {
      this.taskQueries.push({ scoped: true, params });
      return {
        rows: [{ task_id: "t-member", project_id: "project-member", name: "Member task" }],
        rowCount: 1,
      };
    }
    if (q.includes("from tk_tasks where")) {
      this.taskQueries.push({ scoped: false, params });
      return {
        rows: [
          { task_id: "t-private", project_id: "project-private", name: "Private task" },
          { task_id: "t-member", project_id: "project-member", name: "Member task" },
        ],
        rowCount: 2,
      };
    }
...
describe("TraKtiMe manual entry project authorization", () => {
  ...
  it("scopes unfiltered task list to member projects for non-admins", async () => {
    const pool = new PoolShim({ memberships: [["project-member", "u-member"]] });
    await withServer(pool, async (base) => {
      const r = await jsonFetch(`${base}/api/traktime/tasks`, {
        headers: { Authorization: bearerFor("u-member") },
      });
      assert.equal(r.status, 200);
      assert.equal(r.body.tasks.length, 1);
      assert.equal(r.body.tasks[0].project_id, "project-member");
      assert.equal(pool.taskQueries[0]?.scoped, true);
    });
  });
});
