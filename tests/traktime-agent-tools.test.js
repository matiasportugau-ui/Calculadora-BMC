...
async function main() {
  ...
  app.get("/api/traktime/month-report", (req, res) => { record(req); res.json({ ok: true, month: req.query.month, tz: "America/Montevideo", report: { totals: { effective_seconds: 1 }, projects: [] }, pdf_url: null, pdf_download_url: `/api/traktime/month-report.pdf?month=${req.query.month}` }); });
  ...
  try {
    ...
    // ── Reads: list_entries / day / month / billable map correctly ──────────
    {
      ...
      assert(
        "month_report GET /month-report + authenticated pdf path",
        calls.at(-1)?.path === "/api/traktime/month-report"
          && month.pdf_url == null
          && month.pdf_download_url === "/api/traktime/month-report.pdf?month=2026-06",
