...
test("POST ai-verify-stop requires auth and non-object body → 400 (#1019)", async () => {
  ...
  const { port, close } = await listenRouter(
    ...
    {
      pool: null,
      runEnviosAiVerify: async () => {
        called += 1;
        return { ok: true, proposal: { ok: true }, evidence: { sources: [] } };
      },
    },
  );
  try {
    const anon = await request(port, "POST", "/api/envios/ai-verify-stop", {
      body: { stop: { cliente: "A" } },
    });
...
test("POST ai-verify-stop returns proposal via DI (human-gate, no Sheets write)", async () => {
  const proposal = {
    ok: true,
    confidence: "alta",
  ...
  const evidence = { sources: [{ id: "ventas_row", content: "x" }], gaps: [] };
  let seen = null;
  const { port, close } = await listenRouter(
    { apiAuthToken: API_TOKEN, databaseUrl: "" },
    {
      pool: null,
      runEnviosAiVerify: async (stop) => {
        seen = stop;
        return { ok: true, proposal, evidence, provider: "stub", rawPreview: "{}" };
      },
    },
  );
  try {
    const res = await request(port, "POST", "/api/envios/ai-verify-stop", {
      token: API_TOKEN,
      body: {
        stop: {
          cliente: "Alvaro",
          pdfText: "x".repeat(25_000),
          paneles: Array.from({ length: 100 }, (_, i) => ({ tipo: "P", i })),
        },
      },
    });
    assert.equal(res.status, 200);
    assert.equal(res.body.ok, true);
    assert.equal(res.body.proposal.confidence, "alta");
    assert.equal(res.body.provider, "stub");
    assert.ok(seen);
    assert.equal(seen.pdfText.length, 20_000, "pdfText must be bounded");
    assert.equal(seen.paneles.length, 80, "paneles must be bounded");
  } finally {
    await close();
  }
});

test("POST ai-verify-stop maps ai_failed to 502", async () => {
  const { port, close } = await listenRouter(
    { apiAuthToken: API_TOKEN, databaseUrl: "" },
    {
      pool: null,
  ...
  try {
    const res = await request(port, "POST", "/api/envios/ai-verify-stop", {
      token: API_TOKEN,
      body: { stop: { cliente: "A", rawSheetText: "ENCARGO pdf" } },
    });
    assert.equal(res.status, 502);
    assert.equal(res.body.error, "ai_failed");
  } finally {
    await close();
  }
});
