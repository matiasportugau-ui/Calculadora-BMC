...
async function run() {
  ...
  // Verifies the lib delegation path catches and surfaces a stable details field.
  // In envs with no agent providers configured, err.errors may be an empty array.
  {
    ...
    try {
      ...
      try {
        ...
        assert(
          ...
          { status, ok: json?.ok, hasDetails: Array.isArray(json?.details), detailCount: json?.details?.length },
          { status: 503, ok: false, hasDetails: true },
        );
      } finally {
        server.close();
      }
