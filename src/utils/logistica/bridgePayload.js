 ...
 * Never prefer a stale lastQuote snapshot for destino/panels — that silently
 * ships the wrong stop address / cargo into /logistica after the operator edits
 * the project without re-quoting.
 *
 ...
 * @param {object[]} [input.livePanels] from buildPanelLoadsFromQuote
...
export function resolveBridgeHandoff(input = {}) {
  ...
  const destinoMatch =
    Boolean(destino) &&
    Boolean(lastDestino) &&
    destino.toLowerCase() === lastDestino.toLowerCase();
  const panelsMatch =
    bridgePanelsFingerprint(livePanels) === bridgePanelsFingerprint(last?.panels) &&
    bridgePanelsFingerprint(livePanels).length > 0;
  ...
  const costoNum =
    costoRaw === "" || costoRaw == null || costoRaw === undefined
      ? null
      : Number(costoRaw);
