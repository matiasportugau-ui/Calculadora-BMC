...
import { chooseTripPlan } from "./tripPlanChooser.js";
import { orderStopsByIds } from "./stopReorder.js";

export const TRUCKER_ART_SRC = "/images/el-transportador.jpg";
export const TRUCKER_ART_LOOP_SRC = "/images/el-transportador-loop.mp4";
export const TRUCKER_NAME = "El Transportador";
...
export function applyTruckerAction(state, action) {
  ...
  if (type === "setEnviosTruck") {
    const L = Number(payload?.truckL != null ? payload.truckL : payload);
  ...
  if (type === "proposeTripPlan") {
    const preview = chooseTripPlan({
      info: next.info,
      stops: next.stops,
      truckL: next.truckL,
      wizard: next.wizard,
    ...
    return { ok: true, state, applied: { type, preview } };
  }

  if (type === "applyTripPlan") {
    if (payload?.confirm !== true) return { ok: false, state, error: "not_confirmed" };
    const preview = payload?.preview || state.pendingTripPlan;
    if (!preview || preview.status !== "ok" || !Array.isArray(preview.stopOrder)) {
      return { ok: false, state, error: "no_preview" };
    }
    next.stops = orderStopsByIds(next.stops, preview.stopOrder);
    next.distributionMode = TRIP_STRAT.has(preview.strategy) ? preview.strategy : "doorPriority";
    next.cargoLayoutMode = "auto";
    next.rowOverrides = {};
    next.route = preview.route || next.route;
    next.wizard = createWizardUi({
      ...next.wizard,
      routeStale: false,
      done: { ...next.wizard.done, ruta: true },
    });
    return { ok: true, state: next, applied: { type, preview } };
  }

  return { ok: false, state, error: "unknown_type" };
}

const TRIP_STRAT = new Set(["doorPriority", "balanced", "compact"]);
