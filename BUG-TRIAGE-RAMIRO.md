...
- **(e) `web` de anclajes/fijaciones — sin columna en la Matriz (hallazgo 07/06, verificación post-merge WOLF-0002):** la pestaña de anclajes (*R y C Tornillos*) solo tiene `Costo USD EX IVA` / `Margen %` / `Ganancia` / `VENTA USD EX IVA` / `Consumidor Final` — **no hay columna web**. En el canal web (Shopify) estas fijaciones de bajo precio se publican **por paquete de 10 unidades** (verificado en descripciones: Caballete $9.20/pack, Taco 3/8 $8.70/pack, Kit Anclaje a H° $11.80/pack, Tuerca 3/8 $2.00/pack, etc.). Por eso el `web` por-unidad cargado en `constants.js` (anclajes ~×1.34 sobre `venta`) **no tiene fuente única hoy**. **Decisión D6 (Matias 07/06): la verdad del `web` de fijaciones = publicaciones de ML/Shopify.** Pendiente extraerlos corriendo la API con creds ML (`/api/...` + OAuth) o vía el MCP de Shopify, normalizando el pack→unidad. Los `venta`/`costo` LOCAL de los anclajes ya quedaron validados en WOLF-0002 (no se tocan).
...
### LOCK
- Plan patch: cargar (a) GSDECAM100, (b) familia GLDCAM Isodec por espesor 100–250 reemplazando el `_all`, (c) set Isoroof 100, (d) según respuesta D2 — con SKUs corregidos (no heredar los clonados de la Matriz). (e) extraer `web` de fijaciones desde ML/Shopify (D6) y normalizar pack→unidad antes de cargar.
...
6. **D6 — DECIDIDA (07/06):** fuente de verdad del **`web` de fijaciones/anclajes = ML/Shopify** (la pestaña de anclajes no tiene columna web; el canal vende en packs de 10). Pendiente operativo: extraer los precios de ML/Shopify, normalizar pack→unidad y cargarlos vía pipeline (WOLF-0003 (e) / WOLF-0004). No bloquea: la calc opera con lista LOCAL.
