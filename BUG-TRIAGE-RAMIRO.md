...
|---|---|---|---|---|---|
| WOLF-2026-0001 | Familia ISOFRIG completa ausente del catálogo | DATA | S2 | RESUELTO (PR #332 mergeado; web+costo+venta cargados) | GC-0001 verde (ambas listas) |
...
**Estado:** RESUELTO (PR #332 mergeado a main 13/06/2026; `web`+`costo`+`venta` cargados; GC-0001 verde en ambas listas; smoke UI verde en preview) | **Clase:** DATA | **Severidad:** S2 | **Reportó:** Ramiro Amaral, 02/06/2026
...
- **(a) GSDECAM100 — ✅ CARGADO** (`constants.js` `gotero_superior.ISODEC[100]`). `venta` 39.468 / `web` 46.046 verbatim de la Matriz (ledger v0.6). SKU corregido `GSDECAM100` (la Matriz clona `GSDECAM80`). `costo: null` — celda Matriz `#REF!` (WOLF-0004), `costo` se lee con `?? 0`. Hotfix 13/06: se movió desde `ISODEC_PIR[100]`, una rama no alcanzable por cotizaciones automáticas (`ISODEC_EPS` resuelve familia `ISODEC`; PIR no tiene 100 mm). `GC-0003` valida ahora `calcTechoCompleto(ISODEC_EPS 100, inclGotSup=true)`.
- **(b) Goteros laterales de cámara Isodec por espesor — ✅ CARGADO** (`gotero_lateral_camara.ISODEC` e `ISODEC_PIR`, 100/150/200/250). `web` verbatim: 27.664 / 28.91 / 43.274 / 37.59. Colapso `_all` (SKU genérico) **eliminado** (`grep -c` del SKU genérico = 0). SKUs corregidos `GLDCAM100..250`. `venta`/`costo: null` — la Matriz/ledger **no** proveen esas columnas por espesor; `p()` cae a `web` (lista activa default = web). `duda abierta`: completar `venta`/`costo` desde la Matriz (WOLF-0004).
  - **D3 (`duda abierta`, sin resolver):** 200 mm (43.274) > 250 mm (37.59); filas 150/200/250 "REVISAR - inabilitado en ML". Valores cargados **verbatim** (no se inventó/corrigió). Pendiente confirmación de Ramiro/Matias.
  - **PIR (resuelto v0.7.1, decisión take-control 11/06):** los paneles ISODEC_PIR (50/80/120) **no tienen fila propia** de gotero lateral de cámara en la Matriz (el producto solo existe en medidas EPS 100–250). En vez de dejarlos sin accesorio, se **restauró el fallback `_all`** con SKU corregido `GLDCAMPIR` y los valores genéricos previos del repo (venta 26.51 / web 30.92 / costo 23.86, **no inventados**). Así PIR sigue resolviendo la línea del BOM y `grep GLDCAM-DC` queda en 0. GC-0003 fija el comportamiento (resuelve vía `_all`, SKU ≠ GLDCAM-DC).
- **(c) Accesorios Isoroof 100 mm — ⛔ BLOQUEADO (dato).** La Matriz reusa SKUs del 80 (`GFSUP80`/`GL80`/`GFS80`) para las filas de 100 mm y **no se proveyó el CSV BROMYROS en sesión**. Guardrail "nunca inventar precio" → no se cargan entradas en $0/null vivas (riesgo de cotización a $0). Acción: Matias pega las filas 100 mm de la Matriz → se cargan con SKUs corregidos `GFSUP100`/`GL100`/`GFS100`.
- **(d) Gotero superior Isodec PIR 120 — ⏭️ SKIP.** D2 sin resolver (¿SKU nuevo o `GF120DC` usado como superior?). Per goal: si D2 no está resuelto al correr, se omite y se registra. Sin cambios.
...
- Plan raíz (3 etapas, absorbe las "Tareas Profundas" de Ramiro + ítems 8–12 de las capturas 05/06). **Estado actualizado v0.5:**
  ...
  2. **Diff determinístico** (script, no LLM) — 🟢 **HECHO (v0.7, etapa 2)**: nuevo `scripts/catalog-diff.mjs` + workflow `.github/workflows/catalog-diff.yml` (cron diario 09:00 UTC + trigger en PRs que tocan `src/data/constants.js`). Compara catálogo (`constants.js`) ↔ MATRIZ viva (CSV `GET /api/actualizar-precios-calculadora`, reutiliza el service-account ya montado en prod — sin credenciales nuevas) y **falla (exit 1) en S1**: divergencia relativa > umbral (default 1 %, piso absoluto 0.01 USD) o valor de catálogo **fuera de banda de magnitud** por categoría (control de "precios fuera de rango" + "errores de formato decimal" que pidió Ramiro). Política de celda vacía: `EMPTY` → `no-source`, **nunca** se compara ni se rellena (atrapa el mecanismo de WOLF-0002); `0` se trata como placeholder/faltante; `#VALUE!`/`#REF!` → celda-error; SKU duplicado → `duplicate-sku`, no compara nada; coma decimal normalizada con función testeada (`32,84` → 32.84, no 3284). Tests unitarios: `tests/catalogDiff.test.js` (en `test:core`). npm: `catalog:diff` / `catalog:diff:json`. **Gate del PR = baseline-aware** (regresión): corre el diff sobre el `constants.js` de la base y el del HEAD (misma MATRIZ viva) y **falla sólo ante S1 nuevos/peores que introduce el PR**, no ante el drift pre-existente — así el control es adoptable y no bloquea PRs ajenos mientras la etapa 1 sigue pendiente. El **cron diario** mantiene el reporte absoluto (falla ante cualquier S1). **Primer run absoluto vs prod (sample `audit-output/catalog-diff/2026-06-11.md`):** 51 divergencias S1 (mayormente `costo`, drift sistemático catálogo↔MATRIZ ~2.8–8 %) + 4 valores MATRIZ fuera de rango + 6 anclajes `no-sku` + 129 `no-source` (ventas vacías en origen, confirma 20/88·3/88). Resolver las divergencias = etapa 1 (saneamiento de origen, Matias) o `matriz:bake` según fuente de verdad por campo. **No edité** `server/config.js` ni `src/data/matrizPreciosMapping.js` (lane MATRIZ).
...
5. **D5 — RESUELTA (13/06):** au = **1.10 m** (ficha Kingspan), núcleo **PIR todos los espesores**, solo Blanco, **web** ex-IVA (Matriz), **costo** ex-IVA (`.accessible-base/matriz_precios.json`) y **venta** ex-IVA — todo cargado. venta_local estaba VACÍA en el origen (13/170, ninguna ISOFRIG; venta_web 0/170 → WOLF-0004), así que se cargó **venta = costo × 1.15** (markup BMC **confirmado por Matias**, uniforme; reproduce la venta histórica al céntimo en IF60/80/100/150). GC-0001 valida ahora ambas listas (web 76.9454 / venta 58.01 @100 mm). Restan menores: **lmin/lmax** (Bromyros) y **venta/web de perfiles U ISOFRIG** (U 40/60/120/180; se tiene su costo) — no bloquean cotización.
...
## Registro de versión
- **v0.8 (13/06/2026): WOLF-2026-0001 → RESUELTO (PR #332 mergeado).** Familia `ISOFRIG_PIR` cargada completa en `constants.js`: 7 espesores 40–180 con las **3 listas** — `web` ex-IVA (Matriz BROMYROS), `costo` ex-IVA (`.accessible-base/matriz_precios.json`, dato duro) y `venta` ex-IVA = **costo × 1.15** (markup BMC confirmado por Matias; venta_local estaba vacía en el origen → confirma WOLF-0004; el factor reproduce la venta histórica al céntimo). Fila 200 clonada excluida; au 1.10 corregido vs 1.14 legacy por **ficha oficial Kingspan** (D5 resuelta por triangulación ficha↔Matriz↔legacy); perfil U ISOFRIG (80/100/150; 40/60/120/180 omitidos sin error, sin precio); visibilidad `camara_frig` + `presupuesto_libre`. **GC-0001 verde** validando **ambas listas** (web 76.9454 / venta 58.01 @100 mm; 10 m² → 769.45 web / 580.10 venta), GC-0002 sigue verde; lint + suite offline verdes; smoke UI corrido en preview de Vercel. De paso, corregida la aserción stale `tests/toolStats.test.js` (32→41 tools tras PR #328). Pendientes menores (no bloquean): lmin/lmax (Bromyros), venta/web de perfiles U ISOFRIG-específicos. Extra: base de conocimiento `docs/product-catalog/` (97 ítems Shopify real).
- **v0.7.2 (13/06/2026) — hotfix GSDECAM100 resoluble.** `GSDECAM100` estaba bajo `gotero_superior.ISODEC_PIR[100]`, pero ninguna cotización automática lo podía resolver (PIR no tiene 100 mm; `ISODEC_EPS` usa familia `ISODEC`). Se movió a `gotero_superior.ISODEC[100]`, se agregó `MATRIZ_SKU_TO_PATH.GSDECAM100`, y `GC-0003` ahora cubre el flujo real `calcTechoCompleto(ISODEC_EPS 100, inclGotSup=true)` con línea `GSDECAM100`.
- **v0.7 (11/06/2026) — WOLF-0004 etapa 2 SHIPPED (diff determinístico CI/cron).** Nuevos: `scripts/catalog-diff.mjs` (catálogo `constants.js` ↔ MATRIZ viva, no-LLM, determinístico; umbral 1 % + piso 0.01 USD + bandas de magnitud por categoría derivadas de la distribución actual; política `EMPTY`/`0`/`#VALUE!` que evita el backfill de WOLF-0002; detección de SKU duplicado; normalizador de coma decimal testeado), workflow `.github/workflows/catalog-diff.yml` (cron diario absoluto + PR-trigger **baseline-aware** en `src/data/constants.js`/`scripts/catalog-diff.mjs` que falla sólo ante regresiones nuevas vs la base, comentario sticky en PR + artifact, skip elegante si la MATRIZ no responde — reutiliza el service-account de prod, sin secretos nuevos), `tests/catalogDiff.test.js` (en `test:core`), npm `catalog:diff`/`catalog:diff:json`, y sample committeado `audit-output/catalog-diff/2026-06-11.md`. **El primer run sobre prod NO es verde:** 51 divergencias S1 (drift de `costo` catálogo↔MATRIZ ~2.8–8 %) documentadas — exactamente el control que pedía Ramiro; cerrarlas es etapa 1 (saneamiento/decisión de fuente de verdad por campo, Matias) vía `matriz:bake`. Restricción de lane respetada: sin tocar `server/config.js` ni `src/data/matrizPreciosMapping.js`.
- **v0.7.1 (11/06/2026) — fix regresión PIR (decisión take-control de Matias).** El cambio (b) había dejado a los paneles ISODEC_PIR (50/80/120) sin gotero lateral de cámara (las claves EPS 100–250 no matchean espesores PIR → línea omitida del BOM). Se restauró el fallback `_all` en `gotero_lateral_camara.ISODEC_PIR` con **SKU corregido `GLDCAMPIR`** y los valores genéricos previos del repo (venta 26.51 / web 30.92 / costo 23.86, no inventados). `grep GLDCAM-DC` sigue en 0. GC-0003 actualizado (12/12 verde): PIR 80 mm resuelve vía `_all`, SKU ≠ GLDCAM-DC. `venta`/`costo` de la familia ISODEC EPS y `costo` de GSDECAM100 siguen pendientes de la planilla (sin inventar).
- **v0.7 (11/06/2026) — WOLF-0003 EN EVAL (carga de accesorios de borde).** Branch `wolf/0003-accesorios`. Cargado en `src/data/constants.js`: **(a)** `GSDECAM100` (venta 39.468 / web 46.046 verbatim, costo null por `#REF!`, SKU corregido); **(b)** familia `gotero_lateral_camara` Isodec/PIR por espesor 100/150/200/250 (web verbatim 27.664 / 28.91 / 43.274 / 37.59), **reemplazando el colapso `_all`** (SKU genérico eliminado, `grep -c` = 0), SKUs corregidos `GLDCAM100..250`, `venta`/`costo` null (columnas no provistas → `p()` cae a web). **(c)** Isoroof 100 mm **BLOQUEADO** (sin CSV BROMYROS en sesión; no se cargan precios $0/null vivos). **(d)** PIR 120 superior **SKIP** (D2 sin resolver). Golden case `GC-0003.test.mjs` **verde** (9/9), GC-0002 sin regresión, `tests/validation.js` 399/399, lint 0 errores. **Dudas abiertas:** D3 (anomalía 200>250, valores verbatim) y nueva (PIR 50/80/120 sin lateral cámara tras quitar `_all`). Diff acotado a `constants.js` + ledger + eval (sin tocar lane MATRIZ ni valores de PR #276).
