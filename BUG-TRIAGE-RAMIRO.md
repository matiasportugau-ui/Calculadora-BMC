...
5. **D5 — PARCIALMENTE RESUELTA (12-13/06):** au = **1.10 m** (ficha Kingspan), núcleo **PIR todos los espesores**, solo Blanco, **web** ex-IVA (Matriz) y **costo** ex-IVA (`.accessible-base/matriz_precios.json`, dato duro) — todo cargado. **venta_local: VACÍA en el origen digitalizado** (13/170 filas la tienen, ninguna ISOFRIG; venta_web 0/170 → confirma WOLF-0004 "ventas vacías en origen"). Evidencia de política BMC: `venta = costo × 1.15` reproduce la venta histórica al céntimo (IF60/80/100/150 = 47.40/52.29/58.01/70.37; IF40 histórico a ×1.20 = 43.53). **Pendiente:** confirmación de Matias del markup (×1.15, con IF40 a ×1.20) o pegado de la columna venta real del Sheet vivo. Restan también **lmin/lmax** (Bromyros) y **precios venta/web de perfiles U ISOFRIG** (U 40/60/120/180; solo se tiene su costo).
