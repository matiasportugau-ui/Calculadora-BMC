...
description: >
  ...
  Lee BROMYROS: G/J/R tal cual; K ref. c/IVA tal cual; **col S** = venta web **c/IVA** → CSV `venta_web_iva_inc` (tal cual; sin push);
...
- **`npm run matriz:sync-fijaciones-isodec`** — Sincroniza **G/J/R** desde BROMYROS filas **161–166** hacia `FIJACIONES` en `src/data/constants.js` (varilla, tuerca, carrocero, tortugas, taco). `--dry-run` sin escribir. **Arandela plana:** descripción + **ARPLA38** en col D, o fila **167** por defecto si calza; `MATRIZ_ROW_ARANDELA_PLANA` opcional.
...
- **Col G** (`Costo m² USD ex IVA`): **sin IVA** en el número — leer/escribir **tal cual** la celda (nunca ÷ ni × 1,22).
- **Col J** (venta local ex IVA): **tal cual** la celda.
- **Col K** (referencia **c/IVA**): el número **ya incluye IVA** — **tal cual** la celda (nunca convertir).
- **Col R**: venta web lista → CSV `venta_web` y push, **tal cual**.
- **Col S** (`Venta web USD c/IVA`): → CSV `venta_web_iva_inc` **tal cual** (solo export; no push).
