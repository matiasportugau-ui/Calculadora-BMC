...
function runBatchCalc(extracted, usedDefaults) {
  ...
  // techo_fachada must price BOTH surfaces — previously collapsed to solo_techo
  // (~50% underquote) and wrote that total into Admin col J via quote-batch.
  if (escenario === "techo_fachada") {
    ...
    const techoFamilia = techo.familia && techo.familia !== "null" ? techo.familia
      : (usedDefaults.push("panel ISODEC EPS"), "ISODEC_EPS");
    const techoEspesor = techo.espesor || (usedDefaults.push("espesor 100mm"), 100);
    const tipoEst = techo.tipoEst && techo.tipoEst !== "null" ? techo.tipoEst : "metal";
    const paredFamilia = pared.familia && pared.familia !== "null" ? pared.familia
      : (usedDefaults.push("panel ISOPANEL EPS"), "ISOPANEL_EPS");
    const paredEspesor = pared.espesor || (usedDefaults.push("espesor 100mm"), 100);
    try {
      const rT = calcTechoCompleto({
        familia: techoFamilia, espesor: techoEspesor, tipoEst, color: "Blanco",
        largo: techo.largo, ancho: techo.ancho,
        borders: { frente: "none", fondo: "none", latIzq: "none", latDer: "none" },
        opciones: { inclCanalon: false, inclGotSup: false, inclSell: true },
      });
