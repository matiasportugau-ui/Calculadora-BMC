...
/**
 * @param {{ googleAds?: ReturnType<typeof createGoogleAdsClient>, pool?: import('pg').Pool | null }} [deps]
 */
export function createAdsRouter(deps = {}) {
  const router = Router();
  const googleAds = deps.googleAds ?? createGoogleAdsClient({ config, logger: log });

  let _pool = deps.pool === undefined ? null : deps.pool;
  const pool = () => {
    ...
    if (!_pool) {
      ...
      _pool.on("error", (err) => {
        log.warn?.({ err: err?.message, code: err?.code }, "[ads] pg pool idle client error");
      });
    }
    return _pool;
  };
