 ...
 * for Voice Brain Pack tools (sheets/CRM/calc/PDF) executed via executeTool.
 *
 * Auth: same gate as session mint (API_AUTH_TOKEN OR identity JWT with
 * calc:write). Required because #1108/#1110 made this path execute server-side
 * tools that read Admin sheets / quotation registry and can trigger Chromium
 * PDF + GCS/Drive uploads — unauthenticated callers must not reach them.
 */
router.post(
  "/agent/voice/action",
  actionLimiter,
