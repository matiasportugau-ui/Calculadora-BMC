...
router.post('/keywords', intelLimiter, requireMarketing, async (req, res) => {
  const { keyword, term, cluster, family, intent, priority, on_site_gap } = req.body || {};
  const rawKeyword = keyword ?? term;
  if (!rawKeyword || typeof rawKeyword !== 'string' || !rawKeyword.trim()) {
    return res.status(400).json({ error: 'keyword required' });
  }
  try {
    const entry = await addTrackedKeyword({ keyword: rawKeyword, cluster, family, intent, priority, on_site_gap });
