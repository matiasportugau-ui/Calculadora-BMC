...
export default function createTraktimeRouter(config, logger) {
  ...
  /** Admins bypass; members must own a `tk_project_members` row. */
  ...
  router.get(
    ...
    requireDb,
    asyncHandler(async (req, res) => {
      const project_id = trimOrNull(req.query.project_id);
      const isAdmin = isAdminUser(req.user);
      // Non-admins must not list every project's tasks (leaks foreign project_ids).
      if (!isAdmin && !project_id) {
        const { rows } = await pool.query(
          `select t.*
             from tk_tasks t
             join tk_project_members m
               on m.project_id = t.project_id and m.user_id = $1
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      // JSON is read-only; PDF streams from authenticated /month-report.pdf
      // so employee hours are never persisted to the public quote GCS bucket.
