...
if (process.env.NODE_ENV !== 'test') {
  ...
  cron.schedule('0 4 * * *', () => {
    ...
    startKeywordRefresh({ priority: 'P1' }).promise.catch((err) => log.error({ err }, 'scheduled P1 keyword refresh failed'));
  }, { timezone: 'UTC' });

  // Full keyword universe weekly (Sunday 05:00 UTC)
  cron.schedule('0 5 * * 0', () => {
    log.info('scheduled full keyword refresh triggered');
    startKeywordRefresh().promise.catch((err) => log.error({ err }, 'scheduled full keyword refresh failed'));
  }, { timezone: 'UTC' });
