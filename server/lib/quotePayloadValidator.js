...
export function validateAndPreviewQuote(rawPayload) {
  ...
  // LISTA_ACTIVA is a process-global singleton used by p()/calc routes. Preview
  // must never leave it flipped — unauthenticated voice/action buildQuote used
  // to permanently switch web↔venta for the whole Cloud Run instance (Bug DT).
  const prevLista = LISTA_ACTIVA;
  try {
    if (p.listaPrecios) {
      setListaPrecios(p.listaPrecios);
    }
