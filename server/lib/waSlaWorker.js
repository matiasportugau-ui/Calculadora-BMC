...
export async function runWaSlaTickOnce({ logger, pool }) {
  const log = logger || { info() {}, warn() {}, error() {} };
  if (!pool) {
    log.warn("[waSlaWorker] no pool, tick skipped");
    return { enabled: false, detected: 0, resolved: 0, durationMs: 0 };
  }
  ...
  if (!rt.enabled) {
    return { enabled: false, detected, resolved, durationMs: Date.now() - t0 };
  }
  ...
  const result = { enabled: true, detected, resolved, durationMs: Date.now() - t0 };
  if (detected > 0 || resolved > 0) {
    log.info(result, "[waSlaWorker] tick");
  }
  return result;
}
...
export function startWaSlaWorker({ logger, pool }) {
  ...
  async function tick() {
    ...
    running = true;
    try {
      await runWaSlaTickOnce({ logger: log, pool });
    } catch (e) {
