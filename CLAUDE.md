...
| `src/utils/scenarioOrchestrator.js` | Generic scenario execution (replaces hardcoded if/else) | ~140 |
...
| `src/utils/pdfGenerator.js` | html2pdf.js wrapper (margins: 14/12/22/12 mm) | ~82 |
...
- **Calculation orchestration** is in `scenarioOrchestrator.js` — `executeScenario(id, inputs)` replaces the old if/else chain
...
- **PDF margins**: 14mm top, 12mm sides, 22mm bottom (unified between pdfGenerator.js and CSS @page)
