...
- `src/pdf-templates/` — 12 template layouts + `index.js` dispatcher with `buildQuotationModel()` and `LAYOUT_OPTIONS`.
...
- **12 template layouts** in `src/pdf-templates/` — each receives a `QuotationModel` (built by `buildQuotationModel()` in `index.js`) and returns self-contained HTML.
...
- Templates: `simple` (default) + `simple-carbon`/`sage`/`slate`/`warm`/`ocean` variants, `bmc-pdf`, `soft-modern`, `executive-dark`, `blueprint`, `minimalist`, `construction-bold`, plus the `classic` Hoja Visual Cliente.
