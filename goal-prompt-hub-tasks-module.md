...
[INFERENCIA: config.js has googleOauthClientId already exposed (line 175) — reuse it for Tasks OAuth; basis: Sheets uses Google service-account, but OAuth client ID may already be configured for Google OAuth flows.]
...
[HECHO CONFIRMADO: Migration date must be 20260602 (AFTER identity migrations 20260601000004) so FK to identity.users(user_id) resolves correctly.]

[INFERENCIA: server/index.js mounts tasks routes as app.use(tasksRouter) with /api prefix pattern — confirm mount path in 01-architecture.md OpenAPI patch.]
...
[DUDA ABIERTA: Token encryption strategy — application-layer AES-256-GCM (chosen to avoid pgcrypto dependency) — verify no hidden pgcrypto requirement in existing Supabase setup before Phase 1.]
...
- Write 02-mcp-server.md: Tech stack decision + tools table + auth strategy + Cloud Run deploy plan
- Write 03-frontend.md: Component tree + theme tokens + state management + offline IndexedDB queue
...
- Create migration: supabase/migrations/20260602000001_tasks_init.sql (apply-ready, date AFTER identity migrations so FK to identity.users resolves; adds "tareas" to identity.modules matching slug convention)
...
# Scope
IN: docs/hub-tasks-module/ (6 documents), supabase/migrations/*_tasks_init.sql, server/routes/tasks*.js stubs, src/components/hub/tasks/ stubs
...
- docs/hub-tasks-module/02-mcp-server.md — stack decision + tools table + auth strategy + Cloud Run deploy plan
- docs/hub-tasks-module/03-frontend.md — component tree + theme tokens + state management + offline IndexedDB queue
- docs/hub-tasks-module/04-roadmap.md — Phases 0-4, binary exit criteria, time estimates, per-phase risks
- docs/hub-tasks-module/05-decisions.md — ≥6 ADR-lite entries (polling vs middleware, conflict resolution, OAuth storage, token encryption, theme approach, OAuth scope, module slug)
...
- All deliverables paths verified; no collision with existing files
...
# Open Items
- [HECHO CONFIRMADO: config.js exposes googleOauthClientId at line 175 — reuse for Tasks OAuth scope]
- [INFERENCIA: server/index.js mounts tasks routes with /api prefix — confirm mount path in 01-architecture.md OpenAPI patch]
- [DUDA ABIERTA: Cloud Scheduler cron expression and service account permissions for /sync/google-tasks/pull — propose in 04-roadmap.md Phase 1]
- [DUDA ABIERTA: Token encryption with application-layer AES-256-GCM — verify no pgcrypto requirement before Phase 1]
